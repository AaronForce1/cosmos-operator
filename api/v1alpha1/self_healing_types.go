...
// SelfHealSpec is part of a TempoFullNode but is managed by a separate controller, SelfHealingReconciler.
// This is an effort to reduce complexity in the TempoFullNodeReconciler.
// The controller only modifies the TempoFullNode's status subresource relying on the TempoFullNodeReconciler
...
type HeightDriftMitigationSpec struct {
	// If pod's height (eth_blockNumber) falls behind the max height of all pods by this value or more
	// AND the pod reports itself as in-sync (eth_syncing false), the pod is deleted. The
	// TempoFullNodeController creates a new pod to replace it.
	// Pod deletion respects the TempoFullNode.Spec.RolloutStrategy and will not delete more pods than set
	// by the strategy to prevent downtime.
	// Validator pods are never deleted by drift mitigation (double-sign guard); a lagging validator
