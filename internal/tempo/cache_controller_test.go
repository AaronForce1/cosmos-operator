...
func (m *mockReader) Get(ctx context.Context, key client.ObjectKey, obj client.Object, opts ...client.GetOption) error {
	...
	*obj.(*tempov1alpha1.TempoFullNode) = crd
	return m.GetErr
}
...
func TestCacheController_SyncedPods(t *testing.T) {
	...
	now := time.Now()
	inSync := NodeStatus{LatestBlockTime: now}
	var catchingUp NodeStatus
	catchingUp.Syncing = true
	catchingUp.LatestBlockTime = now

	var collector mockCollector
	collector.StubCollection = StatusCollection{
		{Pod: &corev1.Pod{ObjectMeta: metav1.ObjectMeta{UID: "1"}}, Status: inSync, TS: now},
		{Pod: &corev1.Pod{ObjectMeta: metav1.ObjectMeta{UID: "2"}}, Status: catchingUp, TS: now},
		{Pod: &corev1.Pod{ObjectMeta: metav1.ObjectMeta{UID: "3"}}, Status: inSync, TS: now},
		{Pod: &corev1.Pod{ObjectMeta: metav1.ObjectMeta{UID: "should not see me"}}, Status: catchingUp, TS: now},
	}
