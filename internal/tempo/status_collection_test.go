...
func TestStatusCollection_Synced(t *testing.T) {
	...
	coll = StatusCollection{
		{Pod: &corev1.Pod{}, Status: catchingUp, TS: now},
		{Pod: &corev1.Pod{}, Err: errors.New("some error"), TS: now},
		// eth_syncing false but stale latest block: must not count as synced.
		{Pod: &corev1.Pod{}, Status: NodeStatus{LatestBlockTime: now.Add(-10 * time.Minute)}, TS: now},
	}
	...
	var pod corev1.Pod
	pod.Name = "in-sync"
	coll = append(coll, StatusItem{Pod: &pod, Status: NodeStatus{LatestBlockTime: now}, TS: now})
